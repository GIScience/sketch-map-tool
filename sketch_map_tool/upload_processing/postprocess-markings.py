...
def postprocess(feature_collection: FeatureCollection):
    ...
    buffered_geometries = [
        geometry.buffer(-buffer_distance) for geometry in dissolved_geometries
    ]

    # Simplify geometries
    simplified_geometries = [
        geometry.simplify(0.2 * buffer_distance) for geometry in buffered_geometries
    ]

    # Create GeoJSON features
    features = [
        Feature(geometry=mapping(geometry), properties=properties)
