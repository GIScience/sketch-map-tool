...
def generate_pdf(output_path: str,  # pylint: disable=R0914  # noqa: C901
                 ...
                 bbox: Bbox,
                 current_date: str,
                 paper_format: PaperFormat = A4) -> str:
    """
    ...
    rotate_indent = -(adjusted_width + paper_format.compass_scale * 3
                      + 3 * paper_format.indent) * cm
    ...
    if rotate:
        ...
        # Add QR-Code:
        renderPDF.draw(svg_qr, canv, paper_format.qr_y*cm, -(paper_format.width-0.1)*cm)

        # Add scale:
        x_scale_cm = 0.24*paper_format.height+paper_format.font_size/1.5
        canv.rect(x_scale_cm*cm, rotate_indent, scale_length/2*cm,
                  paper_format.scale_height*cm, fill=1)
        canv.rect((x_scale_cm+scale_length/2)*cm, rotate_indent,
                  scale_length/2*cm, paper_format.scale_height*cm, fill=0)
        canv.drawString((x_scale_cm+scale_length/2-paper_format.font_size/25)*cm,
                        rotate_indent-paper_format.height*0.012*cm, scale_text[0])
        canv.drawString((x_scale_cm+scale_length-paper_format.font_size/25)*cm,
                        rotate_indent-paper_format.height*0.012*cm, scale_text[1])

        # Add compass:
        renderPDF.draw(compass,
                       canv,
                       (0.24*paper_format.height+paper_format.font_size/1.5
                        + scale_length + paper_format.font_size/4) * cm,
                       rotate_indent-2*paper_format.compass_scale*cm)
        canv.rotate(-90)
    else:
        # Add copyright information:
        text = canv.beginText()
        text.setTextOrigin((adjusted_width+paper_format.indent)*cm, copyright_text_origin)
        ...
        # Add QR-Code:
        renderPDF.draw(svg_qr, canv, (adjusted_width+paper_format.indent)*cm, paper_format.qr_y*cm)

        # Add scale:
        canv.rect((adjusted_width+paper_format.indent)*cm,
                  0.297*paper_format.height*cm,
                  scale_length/2*cm,
                  paper_format.scale_height*cm,
                  fill=1)
        canv.rect((adjusted_width+paper_format.indent+scale_length/2)*cm,
                  0.297*paper_format.height*cm,
                  scale_length/2*cm,
                  paper_format.scale_height*cm,
                  fill=0)
        canv.drawString((adjusted_width
                         + paper_format.indent+scale_length/2 - paper_format.font_size/25)*cm,
                        0.285*paper_format.height*cm,
                        scale_text[0])
        canv.drawString((adjusted_width+paper_format.indent
                         + scale_length - paper_format.font_size/25)*cm,
                        0.285*paper_format.height*cm,
                        scale_text[1])

        # Add compass:
        renderPDF.draw(compass, canv, (adjusted_width+paper_format.indent)*cm,
