...
def generate(paper_format: PaperFormat,
    ...
    if os.path.exists(pdf_path) and not has_failed(pdf_path):  # PDF has already been generated
        return pdf_path
...
def get_result_path(paper_format: PaperFormat,
                    bbox: Bbox,
                    output_path: str,
                    date: str = str(datetime.date(datetime.now()))) -> str:
    """
...
def get_status_link(paper_format: PaperFormat,
                    bbox: Bbox,
                    date: str = str(datetime.date(datetime.now()))) -> str:
    """
