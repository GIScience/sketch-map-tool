...
from sketch_map_tool.analyses.helpers import (
    AnalysisResult,
    add_one_day,
    get_result_path,
)
...
def run_for_single_bbox(
    ...
    result = json.loads(
        requests.get(
            OHSOME_API + "/elementsFullHistory/bbox", params, timeout=TIMEOUT_REQUESTS
        ).text
    )
    if "status" in result.keys() and result["status"] == 503:
        update_progress(
            result_path=status_path, update=STATUS_ERROR_OHSOME_NOT_AVAILABLE
        )
        return
    ...
    currentness_amenity = CurrentnessAnalysis(
        full_history_whole_time, session_id + "/", status_path, key="amenity"
    )
    currentness_highway = CurrentnessAnalysis(
        full_history_whole_time, session_id + "/", status_path, key="highway"
    )
    completeness_amenity = CompletenessAnalysis(
        bbox,
    ...
    completeness_highway = CompletenessAnalysis(
        bbox,
        time_str_full_yearly,
        session_id + "/",
        status_path,
    ...
    landmark_density = LandmarkAnalysis(
        bbox, time_str_current, session_id + "/", status_path
    )
    ...
    write_results_to_json(
        bbox, results, export_path_pdf.replace(output_path + "/", ""), export_path_json
    )
...
def run_preparations_and_analyses(bboxes_input: List[Bbox], output_path: str) -> None:
    ...
    try:
        metadata = requests.get(
            OHSOME_API + "/metadata", timeout=TIMEOUT_REQUESTS
        ).json()
    ...
    time_str_full_yearly = (
        metadata["extractRegion"]["temporalExtent"]["fromTimestamp"][0:10]
        + "/"
        + metadata["extractRegion"]["temporalExtent"]["toTimestamp"][0:10]
        + "/P1Y"
    )
    time_str_full_yearly = add_one_day(
        time_str_full_yearly
    )  # necessary in case the data available
    ...
    time_str_current = (
        time_str_whole_time[11:21] + "/" + time_str_whole_time[11:21] + "/P1Y"
    )

    for bbox in bboxes_input:
        session_id = (
            datetime.now().strftime("%d_%m_%Y_%H_%M_")
            + bbox.get_str(mode="minus")
            + str(random.randint(10000, 99000))  # nosec
            + "_"
        )
        analyses_process = Process(
            target=run_for_single_bbox,
            args=(
                bbox,
                session_id,
                output_path,
...
def run(bboxes_input: List[Bbox], output_path: str) -> Dict[str, str]:
    ...
    Process(
        target=run_preparations_and_analyses, args=(bboxes_input, output_path)
    ).start()
    results = dict()
    for bbox in bboxes_input:
        ...
        status_path = get_result_path(bbox, output_path)
        update_progress(
            result_path=status_path, update=STATUS_UPDATES_ANALYSES["start"]
        )
    return results
