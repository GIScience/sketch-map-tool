...
def add_result_page(result: AnalysisResult,
                    canv: canvas.Canvas,
                    title: str,
                    infotext: str,
                    move_info_up: int = 0,
    ...
    text = "<b>Result:</b><br/>"+result.message.replace("recommendations", "the suggestion")

    if result.suggestion != "":
        text += "<br/><br/><b>Suggestion:</b><br/>"+result.suggestion
    image = RESOURCE_FOLDER+f"light_{result.level.value}.jpg"
    canv.drawImage(image, CONTENTS_X, TRAFFIC_LIGHT_Y, TRAFFIC_LIGHT_WIDTH, TRAFFIC_LIGHT_HEIGHT,
                   showBoundary=False)
    ...
    canv.roundRect(INFO_BOX_X, INFO_BOX_Y, INFO_BOX_WIDTH, INFO_BOX_HEIGHT, 4, stroke=1, fill=0)
    info = Paragraph("<b>Information about this analysis:</b><br/>"+infotext, style=style)
    w, h = info.wrapOn(canv, INFO_BOX_WIDTH-50, INFO_BOX_HEIGHT-25)
    info.drawOn(canv, INFO_BOX_X+25, INFO_BOX_Y+250-h+move_info_up)


def add_overview_page(results: List[AnalysisResult],
    ...
    canv.drawString(CONTENTS_X, RESULT_STRING_1_Y, "The general Sketch Map Fitness Score is: ")
    ...
    canv.drawString(CONTENTS_X+str_width, RESULT_STRING_2_Y, score_string+" fitness")
    str_width_2 = stringWidth(score_string+" fitness", RESULT_STRING_FONT_HIGHLIGHT,
                              RESULT_STRING_FONT_SIZE)
    canv.setFont(RESULT_STRING_FONT, RESULT_STRING_FONT_SIZE)
    canv.drawString(CONTENTS_X+str_width+str_width_2, RESULT_STRING_2_Y, " for usage in sketch "
                                                                         "maps.")
    canv.drawString(CONTENTS_X, SUGGESTION_HEADLINE_Y, "You might want to consider the following "
                                                       "suggestions:")
    suggestions = ""
    ...
    style = ParagraphStyle(name="_", fontSize=SUGGESTIONS_FONT_SIZE, fontName=SUGGESTIONS_FONT)
...
def add_plot(canv: canvas.Canvas,
             plot_path: str,
             description: str,
             plot_width: int,
             plot_y: int = 310) -> None:
    ...
    ratio = height / width
    canv.drawImage(plot_path, page_width/2-plot_width/2, plot_y, plot_width, plot_width*ratio,
                   showBoundary=True)
...
def create_title(canv: canvas.Canvas, bbox: Bbox) -> None:
    ...
    link_information = Paragraph("Created using the <link href='https://www.geog.uni-heidelberg.de/"
                                 "gis/sketchmaptool.html' color='blue'><u>Sketch Map Tool</u>"
                                 "</link>", style=style)
    style = ParagraphStyle(name="_", fontSize=NOTES_SIZE, fontName=FONT)
    license_information = Paragraph("This tool accesses OpenStreetMap data, which is partly "
    ...
    canv.drawImage(RESOURCE_FOLDER+"world_map.jpg", CONTENTS_X, MAP_Y, MAP_WIDTH, MAP_HEIGHT,
                   showBoundary=True)

    canv.setFont(FONT, INFO_SIZE)
    canv.drawString(CONTENTS_X, BBOX_INFO_Y, f"Study region: {bbox.get_str(mode='comma')}")
...
def create_report(results: List[AnalysisResult], plots_path: str, output_path: str,
    ...
    for result in results:
        ...
        elif "Currentness" in result.title_for_report and "amenity" in result.title_for_report:
            plot_label = "Currentness of amenity data"
            info_text = info_dict["le-am"]
        elif "Currentness" in result.title_for_report and "highway" in result.title_for_report:
            plot_label = "Currentness of street data"
            info_text = info_dict["le-hw"]
        elif "Saturation" in result.title_for_report and "amenity" in result.title_for_report:
            plot_label = "Development of amenity feature mapping"
            info_text = info_dict["sat-am"]
            plot_width = 250
            plot_y = 410
            move_info_up = 100
        elif "Saturation" in result.title_for_report and "highway" in result.title_for_report:
        ...
        if result.corresponding_plot_name != "":
            add_plot(c, plots_path + result.corresponding_plot_name,
                     f"Fig. {fig_counter} {plot_label}", plot_width, plot_y)
            fig_counter += 1
        add_result_page(result, c, result.title_for_report, info_text,
                        chapter_nr=chapter_count, move_info_up=move_info_up)
        add_page_nr(c, page_count)
        page_count += 1
        chapter_count += 1
        c.showPage()
